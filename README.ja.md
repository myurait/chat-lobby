...
- Open WebUI スタックを動かすための Docker Compose
- host 側 TypeScript サービスを直接起動するための Node.js 22 以上
- sync helper と現在の自動テストで使う Python 3
- Open WebUI から host 側 adapter / dispatcher へ渡す共通 bearer token を `.env` に持つこと
...
`.env` の `CHATLOBBY_INTERNAL_API_TOKEN` も Open WebUI コンテナへ渡す。
...
# host 側サービス起動前に .env を現在の shell へ読み込む
set -a
source .env
set +a
...
# Open WebUI に status panel pipe を登録
...
`typecheck` script を使うには、shell から `tsc` が利用可能である必要がある。

## Status Store

status store は、Phase F で追加したワーカー進捗正規化用の sidecar である。

```bash
# Open WebUI から使う場合は host から到達できる bind で起動
STATUS_STORE_HOST=0.0.0.0 node services/status-store/src/server.ts

# smoke test 用イベントを直接投入
...
# 最近の running task を確認
curl "http://127.0.0.1:8791/tasks?state=running&limit=10" \
  -H "Authorization: Bearer ${CHATLOBBY_INTERNAL_API_TOKEN}"
```

各 adapter は `CHATLOBBY_STATUS_STORE_URL` へ best-effort で status event を送信し、
frontdoor で approval 要否を見せられるように `approvalState` を正規化する。
...
同期済み pipe が token を自動取得できない場合は、function valves の `api_bearer_token` を設定して再同期する。

## Status Panel

pipe を同期した後、Open WebUI で `ChatLobby Status Panel` モデルを選び、plain text か JSON を送る。
...
単一レコードを見たい場合は、次のように `statusId` を送る。
...
panel は、正規化済みの `worker`、`state`、`approval`、`step`、`updated_at` を会話へ返す。
...
11. status store を `STATUS_STORE_HOST=0.0.0.0` で起動し、`chatlobby_status_panel` を同期したうえで、Claude / Codex / knowledge task 実行後に `ChatLobby Status Panel` モデルが `worker`、`state`、`approval`、`step` を表示することを確認する。

#!/usr/bin/env python3
"""Register or update the ChatLobby status panel pipe in Open WebUI."""
...
DEFAULT_PIPE_FILE = ROOT_DIR / "tools" / "openwebui" / "chatlobby_status_panel_pipe.py"


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Register or update the ChatLobby status panel pipe in Open WebUI.")
    ...
    parser.add_argument("--function-id", default="chatlobby_status_panel")
    parser.add_argument("--name", default="ChatLobby Status Panel")
    parser.add_argument(
        "--description",
        default="Inspect running and recent ChatLobby worker state from the status store.",
    )
